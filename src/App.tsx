...
import devImagem from './assets/devmemory_logo.png';
...
const App = ()=>{
  ...
  const [gridItems, setGridItemns] = useState<GridItemType[]>([]);
    ...
    if(shownCount === 2) {
      ...
      if(opened.length === 2) {
        ...
        if(opened[0].item === opened[1].item) {
          ...
          setGridItemns(tmpGrid);
          setShownCount(0);
        } else {
          //v2 - if  they anot are NOT equal close all "shown"
         setTimeout(()=>{
          let tmpGrid = [...gridItems];
          for(let i in tmpGrid) {
            tmpGrid[i].shown = false;
  ...
  return (
    <C.Container>
      <C.Info>
        <C.LogoLink href="">
          <img src={devImagem} width="200" alt="" />
        </C.LogoLink>
